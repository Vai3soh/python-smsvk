...
async def wait_code(func, args, kwargs, timeout):
    ...
    while True:
        ...
        logging.info(
            f"{count}. Start while loop time - {datetime.fromtimestamp(start).strftime(date_format)}")
        ...
        for resp in r:
            ...
            if 'STATUS_OK' in resp[-1]:  # STATUS_OK
                resp_new.append(
                    (resp[0], resp[2], int(resp[-1].split(':')[-1])))
                args = (
                    args[0], args[1], [
                        i for i in args[2] if i[0] != resp[0]])
        ...
        if not args[2]:
            logging.info(
                f"Quit from while loop time - {datetime.fromtimestamp(time.time()).strftime(date_format)}")
            return resp_new
...
def get_code_activation_mode(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        logging.info(
            f"Results from waite_code received len(r) = {len(r)}, r = {r}")
        code_true = [(i[0], i[1], i[2], 6)
                     for i in r if isinstance(i[2], int)]  # set 6
        logging.info(f"Results sms: code_true = {code_true}")
        other = [
            (i[0],
             i[1],
                i[2],
                8) for i in r if i[2] != 'STATUS_WAIT_RESEND' and not isinstance(
                i[2],
                int)]  # and i[3] != 'STATUS_CANCEL' and i[3] != 'NO_ACTIVATION'
